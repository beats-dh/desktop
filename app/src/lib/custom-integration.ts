 ...
 * placeholder when parsing the saved arguments string; as a defensive measure
 * we also collapse an argument that is exactly the quoted placeholder back to
 * the bare path.
 *
...
export function expandTargetPathArgument(
  ...
  return args.map(arg => {
    // Defensive: if the whole argument is exactly the quoted placeholder,
    // collapse it back to the bare path. Otherwise preserve any user-provided
    // quoting and replace the placeholder in place.
    if (
